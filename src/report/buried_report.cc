...
void BuriedReportImpl::Start() {
    ...
    timer_->async_wait(
        Context::GetGlobalContext().GetReportStrand().wrap(
            [this](const boost::system::error_code& ec) {
                if (ec) {
                    logger_->error(
                        "BuriedReportImpl::Start error: {}",
                        ec.message());
...
BuriedDb::Data BuriedReportImpl::MakeDbData_(const BuriedData& data) {
    ...
    db_data.priority = static_cast<std::int32_t>(data.priority);

    db_data.timestamp = static_cast<std::uint64_t>(
    ...
    SPDLOG_LOGGER_INFO(
        logger_,
        "BuriedReportImpl insert data size: {}",
        db_data.content.size());

    return db_data;
}

std::string BuriedReportImpl::GenReportData_(const std::vector<BuriedDb::Data>& datas) {
    ...
    for (const auto& data : datas) {
        ...
        SPDLOG_LOGGER_INFO(
            logger_,
            "BuriedReportImpl report data content size: {}",
            data.content.size());
...
void BuriedReportImpl::NextCycle_() {
    ...
    timer_->async_wait(
        [this](const boost::system::error_code& ec) {
            if (ec) {
                logger_->error(
                    "BuriedReportImpl::NextCycle_ error: {}",
                    ec.message());
                return;
            }
