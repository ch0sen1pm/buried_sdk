...
static void WriteRegister(const std::string& key,
                          const std::string& value) {
    HKEY h_key;

    LONG ret = ::RegCreateKeyExA(
        HKEY_CURRENT_USER, "Software\\Buried", 0, NULL,
        REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &h_key, NULL);
...
static std::string ReadRegister(const std::string& key) {
    ...
    LONG ret = ::RegOpenKeyExA(
        HKEY_CURRENT_USER, "Software\\Buried", 0, KEY_READ, &h_key);
...
static std::string GetSystemVersion() {
    ...
    ::GetVersionExA(
        reinterpret_cast<OSVERSIONINFOA*>(&os_version_info));
...
std::string CommonService::GetProcessTime() {
    ...
    sprintf_s(buf, "%04d-%02d-%02d %02d:%02d:%02d.%03d",
              create_sys_time.wYear, create_sys_time.wMonth,
              create_sys_time.wDay, create_sys_time.wHour,
...
std::string CommonService::GetRandomId() {
    ...
    std::generate_n(
        std::back_inserter(result), kLength,
        [&]() { return kChars[dist(rng)]; });
